...
class FDTD:
    ...
    def _apply_cpml_h(self, dEz_dx, dEy_dx, dEx_dy, dEz_dy, dEy_dz, dEx_dz):
        """Add psi corrections to H using the raw finite differences (already divided by lengths)."""
        xp, n = self.xp, self.npml
        cH = self.cH
        # --- x faces: derivatives along x live on cell index i (nx-1 cells)
        for side, sl in (("lo", slice(0, n)), ("hi", slice(-n, None))):
            b, c = self.pml[("x", side)]["h"]
            b = xp.asarray(b if side == "lo" else b[::-1].copy())[:, None, None]
            c = xp.asarray(c if side == "lo" else c[::-1].copy())[:, None, None]
            p = self._psi("Hy_x_" + side, dEz_dx[sl].shape); p *= b; p += c * dEz_dx[sl]; self.Hy[sl] += cH * p
            p = self._psi("Hz_x_" + side, dEy_dx[sl].shape); p *= b; p += c * dEy_dx[sl]; self.Hz[sl] -= cH * p
        for side, sl in (("lo", slice(0, n)), ("hi", slice(-n, None))):
            b, c = self.pml[("y", side)]["h"]
            b = xp.asarray(b if side == "lo" else b[::-1].copy())[None, :, None]
            c = xp.asarray(c if side == "lo" else c[::-1].copy())[None, :, None]
            p = self._psi("Hz_y_" + side, dEx_dy[:, sl].shape); p *= b; p += c * dEx_dy[:, sl]; self.Hz[:, sl] += cH * p
            p = self._psi("Hx_y_" + side, dEz_dy[:, sl].shape); p *= b; p += c * dEz_dy[:, sl]; self.Hx[:, sl] -= cH * p
        for side, sl in (("lo", slice(0, n)), ("hi", slice(-n, None))):
            b, c = self.pml[("z", side)]["h"]
            b = xp.asarray(b if side == "lo" else b[::-1].copy())[None, None, :]
            c = xp.asarray(c if side == "lo" else c[::-1].copy())[None, None, :]
            p = self._psi("Hx_z_" + side, dEy_dz[:, :, sl].shape); p *= b; p += c * dEy_dz[:, :, sl]; self.Hx[:, :, sl] += cH * p
            p = self._psi("Hy_z_" + side, dEx_dz[:, :, sl].shape); p *= b; p += c * dEx_dz[:, :, sl]; self.Hy[:, :, sl] -= cH * p
    ...
    def _apply_cpml_e(self, dHz_dx, dHy_dx, dHx_dy, dHz_dy, dHy_dz, dHx_dz):
        ...
        xp, n = self.xp, self.npml
        for side, sl in (("lo", slice(0, n)), ("hi", slice(-n, None))):
            b, c = self.pml[("x", side)]["e"]
            bb = (b[1:n + 1] if side == "lo" else b[1:n + 1][::-1]).copy()      # interior nodes 1..n from the boundary
            cc = (c[1:n + 1] if side == "lo" else c[1:n + 1][::-1]).copy()
            b_, c_ = xp.asarray(bb)[:, None, None], xp.asarray(cc)[:, None, None]
            p = self._psi("Ey_x_" + side, dHz_dx[sl].shape); p *= b_; p += c_ * dHz_dx[sl]; self.Ey[1:-1, :, 1:-1][sl] -= self.cEy[1:-1, :, 1:-1][sl] * p
            p = self._psi("Ez_x_" + side, dHy_dx[sl].shape); p *= b_; p += c_ * dHy_dx[sl]; self.Ez[1:-1, 1:-1, :][sl] += self.cEz[1:-1, 1:-1, :][sl] * p
        for side, sl in (("lo", slice(0, n)), ("hi", slice(-n, None))):
            b, c = self.pml[("y", side)]["e"]
            bb = (b[1:n + 1] if side == "lo" else b[1:n + 1][::-1]).copy(); cc = (c[1:n + 1] if side == "lo" else c[1:n + 1][::-1]).copy()
            b_, c_ = xp.asarray(bb)[None, :, None], xp.asarray(cc)[None, :, None]
            p = self._psi("Ez_y_" + side, dHx_dy[:, sl].shape); p *= b_; p += c_ * dHx_dy[:, sl]; self.Ez[1:-1, 1:-1, :][:, sl] -= self.cEz[1:-1, 1:-1, :][:, sl] * p
            p = self._psi("Ex_y_" + side, dHz_dy[:, sl].shape); p *= b_; p += c_ * dHz_dy[:, sl]; self.Ex[:, 1:-1, 1:-1][:, sl] += self.cEx[:, 1:-1, 1:-1][:, sl] * p
        for side, sl in (("lo", slice(0, n)), ("hi", slice(-n, None))):
            b, c = self.pml[("z", side)]["e"]
            bb = (b[1:n + 1] if side == "lo" else b[1:n + 1][::-1]).copy(); cc = (c[1:n + 1] if side == "lo" else c[1:n + 1][::-1]).copy()
            b_, c_ = xp.asarray(bb)[None, None, :], xp.asarray(cc)[None, None, :]
            p = self._psi("Ex_z_" + side, dHy_dz[:, :, sl].shape); p *= b_; p += c_ * dHy_dz[:, :, sl]; self.Ex[:, 1:-1, 1:-1][:, :, sl] -= self.cEx[:, 1:-1, 1:-1][:, :, sl] * p
            p = self._psi("Ey_z_" + side, dHx_dz[:, :, sl].shape); p *= b_; p += c_ * dHx_dz[:, :, sl]; self.Ey[1:-1, :, 1:-1][:, :, sl] += self.cEy[1:-1, :, 1:-1][:, :, sl] * p
    ...
    # ---------------------------------------------------------------- run
    def run(self, exc_fn, nsteps, end_criteria=1e-3, log=None, min_steps=1000, log_every=2000):
        xp = self.xp
        ...
        for n in range(nsteps):
            ...
            # ---- ports: E_new = lb*(E' - f*E_old) + lb*src*E_src  (resistor + Thevenin source)
            for p in self.ports:
                blk = Ez[p.box]
                ...
                Ez[p.box] = blk
                V = -float(xp.sum(Ez[p.ic, p.jc, p.kz] * p.dz))
                k = p.kmid
                i0, i1, j0, j1 = int(p.ix[0]), int(p.ix[-1]), int(p.jy[0]), int(p.jy[-1])
                I = (float(xp.sum(Hx[i0:i1 + 1, j0 - 1, k] * p.lx)) - float(xp.sum(Hx[i0:i1 + 1, j1, k] * p.lx))
                     + float(xp.sum(Hy[i1, j0:j1 + 1, k] * p.ly)) - float(xp.sum(Hy[i0 - 1, j0:j1 + 1, k] * p.ly)))
                p.V.append(V); p.I.append(I)
            if n % 100 == 0:
